...
int lsh_execute(char **args)
{
    ...
    if (num_pipes > 0) {
        ...
        execute_pipeline(commands, num_pipes + 1, run_in_background);
        free(commands);
        return 1;
    } else {
...
void execute_pipeline(char ***commands, int num_commands, int bg) {
        ...
        if (pids[i] == 0) {
            ...
            for (int k = 0; commands[i][k] != NULL; k++) {
                if (strcmp(commands[i][k], "<") == 0) {
                    freopen(commands[i][k + 1], "r", stdin);
                    commands[i][k] = NULL;
                } else if (strcmp(commands[i][k], ">") == 0) {
                    freopen(commands[i][k + 1], "w", stdout);
                    commands[i][k] = NULL;
                } else if (strcmp(commands[i][k], "2>") == 0) {
                    freopen(commands[i][k + 1], "w", stderr);
                    commands[i][k] = NULL;
                }
            }       
    ...
    printf("checking pipe bg");

    if (!bg) {
        int status;

            wpid = waitpid(pids[num_commands - 1], &status, WUNTRACED);
            
        } else {
        printf("Pipeline running in the background\n");
    }
    printf("checking pipe bg after if");
